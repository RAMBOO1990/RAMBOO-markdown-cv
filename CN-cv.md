...
### **海外SPU同款&比价** `2021/02 – 2022/08`

* 项目背景：海外电商业务扩张，需要商品同款能力支持招商业务的缺货、比价判断。由于跨语言，海外电商基础属性等基建状况较差。
* 个人工作：
    + 建设型号、套装、数量、尺寸、重量、容量等SKU粒度比价关键属性，并推动类目、品牌等SPU粒度属性策略优化；自建型号、套装等spu关键属性特征。
    + 建设数量、尺寸、重量、容量、存储、电气指标等可比价关键属性，过滤spu同款粗排不可比价spu。
    + 建设spu同款模型，辅助多语言文本同款匹配效果；建设基于商品范围细分的关键属性配置，并支持不同粒度的属性匹配。配置于TCC平台，与运营标准对齐。
...
* 项目背景：在3c数码家电行业上，以SPU粒度识别同款数码商品
...
### **SPU同款簇建设** `2021/03 – 2022/02`

* 项目背景：识别商品SPU同款，构建商品SPU同款簇，为商品同款推荐、搜索、去重等业务提供SPU同款能力。
* 个人工作：
...
3.  重构商品簇相似服务，减少计算量，提升服务性能

* 成果：梳理不同行业簇特征，准召有明显改善，帮助簇召回效果提升到90%+；SPU商品簇相似服务性能提升明显，QPS从100-提升至500+

### **sku消重簇pipeline建设** `2021/10 – 2022/02`

* 项目背景：依托当前spu簇能力，在切分至细粒度的sku簇，为商品比价业务提供SKU级别的同款能力。
* 个人工作：整体设计sku消重整体方案，搭建簇挂载、sku簇分桶、簇相似、簇特征更新服务，打通整体pipeline流程。
* 成果：以MVP方案搭建sku消重簇，pipeline整体QPS 500+，pct999延时5s-
