...
* 背景：由于同款簇id唯一，同款簇标准不可变，而许多业务场景需要标准可变的同款能力，因此搭建同款list粒度的同款链路。支持猜你喜欢(推荐)、货品丰富度(缺货招商)、新品标签(c端)、spu标准化(b端)等业务。
* 个人工作：同款精排服务开发，同款模型优化，链路搭建。
    + 搭建粗召、粗排、精排、xgb模型服务链路，以及debug平台等配套工具建设。
    + xgb模型特征建设与优化，包含标题图片特征、属性特征、行业特征、多模特征等。
    + 同款规则建设与优化，包含品牌交叉匹配、互斥词、行业特有规则等。
...
* 成果：
    ...
    + 货品丰富度：缺货品线索准确率82.77%->96.37%，商家同款提报召回643.9%->96.82%(2023年)
    + SPU标准化：商品挂载率+72.7%
